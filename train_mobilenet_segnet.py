...
print("loading mobilenet_segnet")
# actual data is input_height=1024, input_width=2048, but using model defaults
mobilenet_segnet = mobilenet_segnet(20)  # n_classes changed from 19 to 20
print("model beginning training is ", mobilenet_segnet.model_name)
time_begin = str(datetime.now()).replace(' ', '')
print("beginning at", time_begin)
checkpoints_path = "/work/LAS/jannesar-lab/mburke/image-segmentation-keras/checkpoints/mobilenet_segnet-"+time_begin+"/"
os.mkdir(checkpoints_path)

mobilenet_segnet.train(
...
print("Evaluating ", mobilenet_segnet.model_name)
# evaluating the model
print(mobilenet_segnet.evaluate_segmentation(inp_images_dir=data_path + "images_prepped_test/",
